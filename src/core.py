...
def generate_dataset(n_instances: int=50, n_timepoints: int=30, random_state: int=42) -> tuple[np.ndarray, np.ndarray]:
    """Generate synthetic time series dataset."""
    return make_example_3_class_dataset(n_instances=n_instances, n_timepoints=n_timepoints, random_state=random_state)
...
def fit_classifier(X_train: np.ndarray, y_train: np.ndarray, n_neighbors: int=1) -> KNeighborsTimeSeriesClassifier:
    """Fit K-Neighbors time series classifier."""
    clf = KNeighborsTimeSeriesClassifier(n_neighbors=n_neighbors)
    clf.fit(X_train, y_train)
    return clf

def evaluate_classifier(clf: KNeighborsTimeSeriesClassifier, X_test: np.ndarray, y_test: np.ndarray) -> dict[str, Any]:
