...
def prepare_annual_sequences(s: pd.Series) -> tuple[np.ndarray, list]:
    ...
    for year in complete_years:
        year_data = df_complete[df_complete["year"] == year].sort_values("month")["value"].values
        if len(year_data) == 12:
...
def main() -> None:
    ...
    if plot:
        ...
        for k in range(cfg.n_clusters):
            centroid = clusterer.cluster_centers_[k, 0, :]
            ax2.plot(
                months, centroid, label=f"Cluster {k}", linewidth=2.5, marker="o", color=colors[k]
            )
